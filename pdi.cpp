...
inline void func(const Mat img, Mat out,int j,int k,int n) {
	for (;j < k; j++) {
		for (int i = 0; i < n; i++) 
			out.at<uchar>(j, i) = 255 - img.at<uchar>(j, i);
	}
}
...
Mat negative_rgb(const Mat& img) {
	...
	if (img.type() == CV_32FC1 || img.type() == CV_8UC1)
		color = false;
	Mat img_out(img.size(), img.type());
	int step = img.rows/ COUNT_OF_THREADS;
	thread threadArray[COUNT_OF_THREADS];
	for (int row = 0, count = 0; count < COUNT_OF_THREADS; row += step, count++)
		threadArray[count] = thread(func, img, img_out, row, row + step, img.cols + color * img.cols * 2);
	for (int count = 0; count < COUNT_OF_THREADS; count++)
